...
const getStats = async () => {
    return await prisma.$transaction(async (tx) => {
        const [totalPosts, totalPublishedPost, totalArchivedPost,
            totalDraftPost, totalComments, approvedComment,
            totalUser, totalAdmin, userCount, totalPostView] =
            await Promise.all([
                await tx.post.count(),
        ...
        return {
            ...
            totalAdmin,
            userCount,
            totalPostView: totalPostView._sum.views,
        }
    })
}
